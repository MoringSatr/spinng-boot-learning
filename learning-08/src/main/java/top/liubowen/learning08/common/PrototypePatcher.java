...
public class PrototypePatcher implements Patcher {
	...
	private static final String INJECTOR_FIELD = "private static final %s %s = %s.build(%s.class, %s.class);";
		...
		if (isPrototype(clazz)) {
			...
			do {
				...
				for (CtField ctField : fields) {
					if (ctField.hasAnnotation(Autowired.class))
						return true;
				}
				t = t.getSuperclass();
	...
	public void patch(CtClass clazz) throws CannotCompileException {
		...
		String injectorClassName = Injector.class.getName();
		String format = String.format(INJECTOR_FIELD, injectorClassName, injectorName, injectorClassName,
				clazz.getName(), Prototype.class.getName());
		clazz.addField(CtField.make(format, clazz));
