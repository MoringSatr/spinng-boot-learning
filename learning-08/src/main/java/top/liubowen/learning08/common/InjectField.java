	...
	public final void inject(Object bean) throws IllegalArgumentException, IllegalAccessException {
		...
		if (cache == null) {
			throw new IllegalArgumentException(
					type().getName() + " doesn`t loaded , beacuse " + name + " name not exist");
		}
		field.set(bean, cache);
	}

	private Object fetch() {
		return StringUtils.isBlank(name) ? SpringBeanContext.fetchBean(type()) : SpringBeanContext.fetchBean(name);
	}

	@Override
	public String toString() {
		return "InjectField{" + "field=" + field + ", name='" + name + '\'' + ", isLazy=" + isLazy + ", nullable="
